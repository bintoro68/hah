                ...
                if (isGroupMsg && isDetectorBadword && !isGroupAdmins && !isAdmin && !isOwner){
                    if (chats.match("anjing") || chats.match("gblk") || chats.match("tolol") || chats.match("kntl")) {
                        if (!isGroupAdmins) {
                            ...
                            }).catch(() => tobz.sendText(from, `Untung Dark BOT Bukan Admin, Kalo Jadi Admin Udah Aku Kick Tuh! 😑`))
                        } else {
        ...
        case '#stikernobg':
            if (isMedia) {
                try {
                    ...
                    var result = await removeBackgroundFromImageBase64({ base64img, apiKey: '5GK2tjirg3VNhXibU8Ju1mKL', size: 'auto', type: 'auto', outFile })
...
🌐️ *URL*: ${url}`
        ...
        case '#infosurah':
            if (!isGroupMsg) return tobz.reply(from, `Perintah ini hanya bisa di gunakan dalam group!`, id)
            if (args.length == 0) return tobz.reply(from, `*_${prefix}infosurah <nama surah>_*\nMenampilkan informasi lengkap mengenai surah tertentu. Contoh penggunan: #infosurah al-baqarah`, message.id)
        ...
        case '#ytmp3':
            ...
            try {
                ...
                if (resp.error) {
                    ...
                    if (Number(filesize.split(' MB')[0]) >= 30.00) return tobz.reply(from, 'Maaf durasi video sudah melebihi batas maksimal!', id)
                    tobz.sendFileFromUrl(from, thumb, 'thumb.jpg', `➸ *Title* : ${title}\n➸ *Filesize* : ${filesize}\n\nSilahkan tunggu sebentar proses pengiriman file membutuhkan waktu beberapa menit.`, id)
        ...
        case '#artinama':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik #limit Untuk Mengecek Kuota Limit Kamu`, id)
            if (args.length == 0) return tobz.reply(from, `Untuk mengetahui arti nama seseorang\nketik #artinama namakamu`, id)
            artinama(body.slice(10))
            .then(async(res) => {
                await tobz.reply(from, `Arti : ${res}`, id)
            })
            break
        case '#zodiak':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
...
➸ *Filesize:* ${filesize}`
        ...
        case '#joox':
            ...
            console.log(music)
            try {
            const music2 = await axios.get('https://mnazria.herokuapp.com/api/jooxnich?search=' + music)
            const { msinger, msong, malbum, imgSrc, mp3Url, msize } = music2.data.result[0]
...
➸ *Wilayah:* +${region_name}`
        ...
        case '#nulis':
            ...
            await tobz.sendImage(from, `${nulisp}`, '', 'Nih gan...', id)
            .catch(() => {
                tobz.reply(from, 'Ada yang Error!', id)
            })
            break
        ...
        case '#snk':
            ...
            break
        default:
            if (!isGroupMsg) return tobz.reply(from, 'Jika Ingin Menggunakan Bot Harap Masuk Ke Dalam Grup Dark BOT, Link Ada Di Bio atau Bisa Mengetik #elainagroup!\nJika Ingin Sewa Bot atau Bikin Bot Harap Ketik *#iklan*', id)
